...
%plik(P) --> tekst(P).
plik(P) --> namespace(M,-1), nl_k, plik(P2), {info('namespace\n'), concat_atom([M,P2],P)}.
plik(P) --> namespace(M,-1), nl_o, {info('namespace\n'), concat_atom([M],P)}.
...
class_nagl(PN) --> ws, "class", odstep_k, wyraz(Nazwa) , odstep, "{", !, nl_k, {concat_atom(['class ',Nazwa, '{', class_cialo, '}'],PN), info(PN)}.
...
%cout(C,Wci) --> ws, "Console.WriteLine(",cout_srodek(S),");", {wciecie(W,Wci), concat_atom([W,'System.out.print(',S,');'],C),info(S)}.
cout(C,Wci) --> ws, "Console.WriteLine(",cout_srodek(S),");", {wciecie(W,Wci), info(S),concat_atom([W,'System.out.print(',S,');'],C),info(S)}.
