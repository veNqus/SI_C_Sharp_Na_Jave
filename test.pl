...
zmienne_z_wart_('') --> "".
zmienne_bez_wart(L) --> ws,nazwa(N), odstep, ",", odstep, zmienne_bez_wart(LZ), {concat_atom([N, ', ', LZ],L)}.
zmienne_bez_wart(L) --> ws,nazwa(N),"(", calkowita(LC), ")", odstep, ",", odstep, zmienne_bez_wart(LZ), {concat_atom([N,'[', LC, '], ',LZ],L)}.
zmienne_bez_wart(N) --> ws,nazwa(N).
zmienne_bez_wart(Z) --> ws,nazwa(N),"(", calkowita(LC), ")", {concat_atom([N,'[', LC, ']'],Z)}.
